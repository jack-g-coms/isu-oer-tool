/*
  ...
  - The values [COMPLETED] on the enum `SectionStatus` will be removed. If these variants are still used in the database, this will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "SectionStatus_new" AS ENUM ('PENDING', 'QUEUED', 'WRITING', 'READY', 'FAILED');
...
ALTER TABLE "Section" ALTER COLUMN "status" SET DEFAULT 'PENDING';
COMMIT;
